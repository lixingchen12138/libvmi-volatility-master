    ...
    *call_conv = *exported = NULL;
    ...
    if (!(flags & (UNDNAME_NO_MS_KEYWORDS | UNDNAME_NO_ALLOCATION_LANGUAGE)))
    {
        if (flags & UNDNAME_NO_LEADING_UNDERSCORES)
        {
            ...
            default: printf("Unknown calling convention %c\n", ch); return FALSE;
            }
        }
        else
        {
    ...
    if (*sym->current == '?' && (sym->current[1] != '$' || sym->current[2] == '?'))
    {
        ...
        case '_':
            ...
            case 'R':
                ...
                default:
                    printf("Unknown RTTI operator: _R%c\n", *sym->current);
                    break;
                }
                break;
...
int main(int argc, char **argv) {
    char buf[2048];
    char *out;
    out = __unDName(buf, argv[1], 2048, malloc, free, UNDNAME_NO_LEADING_UNDERSCORES);
    if(out) puts(buf);
    return !!out;
}